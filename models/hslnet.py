...
class HSLNet(nn.Module):
    ...
    def forward(self, image, boxes, mode):
        ...
        if mode == 'car_test':
            ...
            if any(bs_mean[:, 0] < imh / self.gamma) or any(bs_mean[:, 1] < imw / self.gamma):
                flag = True
                max_scale = torch.cat(((imh / bs_mean[:, 0]), (imw / bs_mean[:, 1]))).max()
                expand_scale = (max_scale - self.gamma) / self.eta + 1
                if expand_scale > 3:
                    expand_scale = 3
