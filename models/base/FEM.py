...
class FEM(nn.Module):
    ...
    def forward(self, q, s):
        ...
        batch_size, shot, channels, height_s, width_s = s.shape
        s = s.flatten(0, 1)
        ...
        v_s = self.common_v(s).view(batch_size, shot, self.inter_channels, height_s, width_s).transpose(1, 2).flatten(2, 4)
        v_s = v_s.permute(0, 2, 1)

        k_x = self.key(s).view(batch_size, shot, self.inter_channels, height_s, width_s).transpose(1, 2).flatten(2, 4)
        ...
        p_s = torch.matmul(attention_s, v_q)
        p_s = p_s.permute(0, 2, 1).contiguous()
        p_s = p_s.view(batch_size, self.inter_channels, shot, height_s, width_s).transpose(1, 2).flatten(0, 1)
        ...
        q_s = torch.matmul(attention_q, v_s)
