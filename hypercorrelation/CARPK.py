...
from datasets import build_car_loader, build_loader
...
from models.base.roialign import ROIAlign
from torchvision.models import resnet

from models.base.feature import extract_feat_res

from functools import reduce
from operator import add

from datasets.utils import NormalSample
...
def main_worker(config):
    ...
    for line in file.readlines():
        ...
        imglist.append(dic)
    exemplars = []
    while len(exemplars) < 12:
        index = random.randint(0, len(imglist))
        if index not in exemplars:
            exemplars.append(index)
    norm = NormalSample()
    s_feats = []
    for i in range(13):
        s_feats.append([])
    for idx in exemplars:
        e_d = imglist[idx]
        img = Image.open(os.path.join(config.DATA.DATA_PATH, 'CARPK_devkit/data/Images/' + e_d["filename"]))
        img = norm(img).unsqueeze(0)

        all_boxes = e_d["boxes"]
        box = torch.tensor(all_boxes[0])
        t, l, b, r = box
        b, r = max(t + 1, b), max(l + 1, r)
        box = torch.tensor([0, l, t, r, b]).unsqueeze(0)
        ...
        roi = 16
        roialign = [ROIAlign(roi, 1. / 8), ROIAlign(roi, 1. / 16), ROIAlign(roi, 1. / 32)]
        query_feats = extract_feats(img, backbone, feat_ids, bottleneck_ids, lids)
        roi_l = 0
        for idx, query_feat in enumerate(query_feats):
            if idx >= roi_ls[roi_l]:
                roi_l += 1
            support_feat = roialign[roi_l](query_feat, box)
            _, c, sh, sw = support_feat.shape
            support_feat = support_feat.view(1, 1, c, sh, sw)
            s_feats[idx].append(support_feat)
    for idx in range(13):
        s_feats[idx] = torch.cat(s_feats[idx], dim=1).cuda()

    data_loader_train = build_loader(config.DATA, 'train', class_remove=True)
    dataloader_test = build_car_loader(config.DATA)
    ...
    lr_scheduler = build_scheduler(config, optimizer, len(dataloader_test))
    ...
    if config.MODEL.RESUME:
        ...
        mae, mse, loss = validate(config, dataloader_test, model, criterion, s_feats)
    ...
    for epoch in range(config.TRAIN.START_EPOCH, config.TRAIN.EPOCHS):
        ...
        if (mae + mse) < (max_accuracy[0] + max_accuracy[1]):
            if epoch % config.SAVE_FREQ == 0 or epoch == (config.TRAIN.EPOCHS - 1):
                save_checkpoint(config, "best", model_without_ddp, max_accuracy, optimizer, lr_scheduler, logger)
            max_accuracy = (mae, mse, loss)
        logger.info(
            f'Min total MAE|MSE|Loss: {max_accuracy[0]:.6f} | {max_accuracy[1]:.2f} | {max_accuracy[2] * 1e5:.2f}')
...
def train_one_epoch(config, model, criterion, data_loader, optimizer, epoch, lr_scheduler):
    ...
    for idx, (samples, boxes, targets, imgids) in enumerate(data_loader):
        ...
        outputs = model(samples, boxes, 'train')
...
@torch.no_grad()
def validate(config, data_loader, model, criterion, s_feats):
    model.eval()
    ...
    for idx, (images, boxes, target, imgids) in enumerate(data_loader):
        ...
        # compute output
        with torch.no_grad():
            output = model(images, s_feats, 'car_test')
        ...
        pred_cnt = output.sum(dim=(1, 2, 3))
        diff = torch.abs(pred_cnt - tarsum)
        mae, mse = diff.mean(), (diff ** 2).mean()
